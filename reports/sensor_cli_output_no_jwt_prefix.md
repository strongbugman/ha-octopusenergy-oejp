# OEJP Sensor CLI Real Account Output — Raw Authorization Header

Change tested: GraphQL requests now send the token directly as the `Authorization` header value, with no `JWT ` prefix.
...
Result: account/profile/billing sensors still work; `intervalReadings` and `halfHourlyReadings` still return `unauthorized` with error code `KT-CT-4501`.
